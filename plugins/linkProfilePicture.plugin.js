...
class linkProfilePicture {
	...
	observer(mutation){
		var x, i = 0,
			...
			wrapper, a, pic, url;
		while(x = ref[i++]) if("DIV" === x.nodeName && 0 === x.className.indexOf("modal-") && (wrapper = x.querySelector(".avatar-1BXaQj.profile-3z9uol")) &&
			(pic = wrapper.querySelector(".image-EVRGPw")) && (url = pic.style.backgroundImage.match(/https.+(?:webp|png|gif)/))){
